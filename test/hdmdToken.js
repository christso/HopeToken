...
contract('HDMDToken', function(accounts) {
   ...
   it('should mint to calling address if minter is authorized', function() {
      ...
      return (HDMDToken.deployed()
            ...
            .then(function(totalSupply) {
               ...
               assert.equal(
                  totalSupply,
                  totalInitialSupply + mintValue,
                  'Minted amount did not increase total supply'
               );
            })
            ...
            .then(function(balanceOfMinter) {
               ...
               assert.equal(
                  balanceOfMinter,
                  initialBalanceOfMinter + mintValue,
                  'Minted amount did not match the sending account balance'
               );
            }) );
   });
   ...
   it('should be able to allocate a batch of tokens', function() {
      ...
      return HDMDToken.deployed()
         ...
         .then(function() {
            ...
            var a = 1;
            assert.equal(
               endingBalances[a] - startingBalances[a],
               values[a],
            ...
            var totalStartingBalance = startingBalances.reduce(
               (a, b) => a + b,
               0
            );
            assert.equal(
   ...
   it('should throw exception if batch transfer called by non-owner', function() {
      ...
      return HDMDToken.deployed()
         .then(function(instance) {
            hdmd = instance;
            return hdmd.batchTransfer([accounts[0]], [1], {
               from: accounts[1]
            });
         })
         ...
         .catch(function(error) {
            if (error.toString().indexOf('invalid opcode') != -1) {
               console.log(
                  'We were expecting a Solidity throw, we got one. Test succeeded.'
               );
            } else {
   ...
   it('should throw exception if mint amount exceeds maximum total supply', function() {});

   it('should throw exception if mint amount results in negative total supply', function() {});
});
